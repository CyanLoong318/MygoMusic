...
- 📋 **播放队列**：点歌排队、自动连播、上一首 / 下一首 / 暂停 / 继续 / 跳票，队列在游戏内可视化
- 🖥️ **游戏内 GUI**：按 `M` 打开搜索点歌界面、`N` 打开队列界面、`,` 打开设置界面
...
- 游戏中按 `M` 打开搜索界面，点一首酷狗/网易云歌 → 全服同时播放；点 B站视频 → 服务端转码 MP3 后分发
